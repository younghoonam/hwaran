  ...
  updateCurrentSection: function () {
    for (let index = 0; index < sectionElements.length; index++) {
      if (this.currentSection >= sectionElements.length - 1) {
        if (
          sectionElements[index].offsetTop <= window.scrollY &&
          window.scrollY <
...
function initInstTweens() {
  ...
  instTweens.stopPowerOnOff = function () {
    ...
    ledWarm.material.emissiveIntensity = 0.1;
    ledWhite.material.emissiveIntensity = 0.1;
...
function initLeds() {
  const onTime = 500;
  const offTime = 200;
  const onEmissiveIntensity = 1;
  const halfOnEmissiveIntensity = 0.5;
  const offEmissiveIntensity = 0.1;
  // LED WARM
...
function initModelDebug() {
  ...
  gui.add(bloomPass, "threshold", 0, 1, 0.001);
  gui.add(bloomPass, "radius", 0, 1, 0.001);
  gui.add(scene, "environmentIntensity", 0, 1, 0.001);
  ...
  gui.addColor(headbandTextMaterial, "color").listen();
}
